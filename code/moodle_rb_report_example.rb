require 'moodle_rb'
require_relative 'report_html'
# require_relative 'send_message'
require 'pry'
require 'HTTParty'
require 'json'

OUTPUT_PATH = ENV['HOME'] + "/code/report"
...
#todo: this already exists in moodle_rb -> utility, so we can use it from there or just extend the gem
...
def send_messages(params)
 response = HTTParty.post(
   'http://localhost:8888/moodle29/webservice/rest/server.php',
   {
     :query => query_hash('core_message_send_instant_messages', @web_services_token),
     :body => {
...
def grades_report
  ...
  # binding.pry
  begin
    ReportHTML.write(class_data["grades"], OUTPUT_PATH + ".html")
  rescue
