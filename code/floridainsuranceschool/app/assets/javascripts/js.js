var clicky_obj = clicky_obj || (function() {
    ...
    function _ins() {
        ...
        this.custom_data = function() {
            ...
            for (var i = 0; i < l; i++) {
                ...
                if (clicky_custom.visitor) {
                    ...
                    if (temp) {
                        data[key] = temp;
                        _self.set_cookie('_custom_data_' + key, _self.enc(temp));
                    }
                }
        ...
        this.set_referrer = function() {
            ...
            if (r) {
                _self.set_cookie('_referrer_og', _self.enc(r), 86400 * 90);
            } else {
        ...
        this.queue_get = function() {
            ...
            return q ? JSON.parse(decodeURIComponent(q)) : _self.queue_default();
        };
        this.queue_set = function(q, ex) {
            for (var i = 0; i < q.heatmap.length; i++)
                q.heatmap[i].href = _self.enc(q.heatmap[i].href);
            for (var i = 0; i < q.events.length; i++) {
                q.events[i].href = _self.enc(q.events[i].href);
                if (q.events[i].title)
                    q.events[i].title = _self.enc(q.events[i].title);
            }
            _self.set_cookie('_eventqueue', _self.enc(JSON.stringify(q)), (ex || 600));
        };
        ...
        this.set_cookie = function(name, value, expires) {
            ...
            var temp = name + "=" + value + ";expires=" + ex.toGMTString() + ";path=/;";
        ...
        this.advanced = function() {
            ...
            var is_download = new RegExp("\\.(7z|aac|apk|avi|cab|csv|dmg|doc(x|m|b)?|epub|exe|flv|gif|gz|jpe?g|js|m4a|mp(3|4|e?g)|mobi|mov|msi|ods|pdf|phps|png|ppt(x|m|b)?|rar|rtf|sea|sit|tar|torrent|txt|wma|wmv|xls(x|m|b)?|xml|zip)$", "i");
